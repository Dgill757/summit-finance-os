...
export default async function GoalsPage() {
  ...
  const [{ data: goals }, { data: monthTransactions }, { data: goalDeposits }] = await Promise.all([
    supabase.from('goals').select('*').eq('user_id', user?.id).order('created_at', { ascending: false }),
    supabase.from('transactions').select('amount').eq('user_id', user?.id).gte('date', monthStart).lte('date', monthEnd),
    supabase.from('goal_deposits').select('id, goal_id, amount, note, created_at'),
  ])
  ...
  return (
    <div className="pb-10">
      ...
      <GoalsContent goals={goals || []} goalDeposits={goalDeposits || []} userId={user?.id || ''} savingsRate={savingsRate} income={income} />
    </div>
  )
}
