    ...
    <div className="space-y-6 px-6 py-6">
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-card">
        ...
        <form onSubmit={addManualAccount} className="mt-4 grid gap-3 md:grid-cols-[1fr,180px,180px,140px]">
          <input value={manualAccountForm.name} onChange={(e) => setManualAccountForm((current) => ({ ...current, name: e.target.value }))} placeholder="Account name" className="w-full px-4 py-3 rounded-xl bg-panel border border-border text-primary text-sm placeholder:text-muted focus:outline-none focus:border-teal/40 focus:ring-1 focus:ring-teal/30 transition-all" />
          <select value={manualAccountForm.type} onChange={(e) => setManualAccountForm((current) => ({ ...current, type: e.target.value }))} className="w-full px-4 py-3 rounded-xl bg-panel border border-border text-primary text-sm focus:outline-none focus:border-teal/40 focus:ring-1 focus:ring-teal/30 transition-all">
            <option value="depository">Checking / Savings</option>
            <option value="credit">Credit Card</option>
            <option value="investment">Investment</option>
            <option value="loan">Loan</option>
          </select>
          <input value={manualAccountForm.starting_balance} onChange={(e) => setManualAccountForm((current) => ({ ...current, starting_balance: e.target.value }))} type="number" step="0.01" placeholder="Starting balance" className="w-full px-4 py-3 rounded-xl bg-panel border border-border text-primary text-sm placeholder:text-muted focus:outline-none focus:border-teal/40 focus:ring-1 focus:ring-teal/30 transition-all" />
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal text-canvas font-semibold text-sm hover:bg-teal/90 transition-all">Create</button>
        </form>
