  ...
  try {
    ...
    for (const item of items) {
      ...
      if (toAdd.length) {
        const inserts = toAdd
          ...
          .map((tx) => ({
            ...
            category: mapPlaidCategory(tx.personal_finance_category?.primary ? [tx.personal_finance_category.primary, tx.personal_finance_category.detailed] : tx.category),
      ...
      for (const tx of toModify) {
        await supabase.from('transactions').update({ amount: tx.amount, name: tx.merchant_name || tx.name, pending: tx.pending, category: mapPlaidCategory(tx.category) }).eq('plaid_transaction_id', tx.transaction_id)
      }
